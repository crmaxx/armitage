...
public class Main implements Runnable, CortanaPipe.CortanaPipeListener {
	...
	public static Object[] setupConnections(String host, String port, String token, String nick) {
		...
		loader.setGlobal("$token",  SleepUtils.getScalar(token));
	...
	public void start(String host, String port, String token, String nick, String[] scripts) {
		/* rock 'n' roll with this big bad puff o stuff */
		try {
			Object conns[] = setupConnections(host, port, token, nick);
	...
	public static void main(String args[]) {
		...
		if (args.length >= 1) {
			try {
				...
				String argz[] = new String[4];
				argz[0] = temp.getProperty("host");
				argz[1] = temp.getProperty("port");
				argz[2] = temp.getProperty("token");
				argz[3] = temp.getProperty("nick");
				...
				cortanaEngine.start(argz[0], argz[1], argz[2], argz[3], scripts);
			}
